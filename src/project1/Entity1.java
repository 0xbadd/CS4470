package project1;public class Entity1 extends Entity {
  ...
  public Entity1() {
    ...
    // This section has the same function as in Entity0, but the values are
    // Entity1's. As you can see,
    ...
    // This send protocol differs from Entity0's, because Entity1 only sends 2
    // packets, and I decided that it would
    // be more efficient if I just made each packet than if I were to run a for
    // loop.
    Packet dtPacket = new Packet(1, 0, minDistance);
    NetworkSimulator.toLayer2(dtPacket);
    dtPacket = new Packet(1, 2, minDistance);
    NetworkSimulator.toLayer2(dtPacket);
    System.out.println("Entity1 Initialization Complete. Distance Table is:");
    System.out.println("Time now is " + NetworkSimulator.getTime() + ".");
    printDT();
  }
  ...
  public void update(Packet p) {
    ...
    // this send protocol is the same as the one in the construstor. It once again
    // differs from Entity0's.
    if (send) {
      Packet dtPacket = new Packet(1, 0, minDistance);
      NetworkSimulator.toLayer2(dtPacket);
      dtPacket = new Packet(1, 2, minDistance);
  ...
  public void linkCostChangeHandler(int whichLink, int newCost) {
    ...
    distanceTable[2][2] = 1;
    distanceTable[3][3] = 999;
    // next, we replace the value of getting to the node with the changed link with
    // the new cost
